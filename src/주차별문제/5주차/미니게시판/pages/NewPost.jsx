...
import { create } from "../data/postsStore";
...
export default function NewPost() {
  ...
  const validate = () => {
    ...
    if (title.length > 100) e.title = "제목은 100자 이하";
  ...
  const onSubmit = (ev) => {
    ...
    const saved = create({ title: title.trim(), body: body.trim() });
    nav(`/posts/${saved.id}`);
  };

  return (
    <>
      <h1 style={{ marginTop: 0 }}>✚ 새 글 작성</h1>
      <form onSubmit={onSubmit}>
        ...
        <p style={{ marginTop: 12 }}>
          ...
          <Link to="/posts" style={linkBtn}>취소</Link>
        </p>
      </form>
    </>
  );
